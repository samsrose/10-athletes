...
export default class Profile extends Component {
  ...
  componentDidMount() {
    ...
    const getUser = axios.get('http://localhost:3001/logged_in', {withCredentials: true})
 ...
 const { sportMatches, rating, sportID, sport } = this.state
  ...
  if (error === ""){
    var url = "http://localhost:3001/users/" + this.state.user.id
    var url2 = "http://localhost:3001/sports/" + bold
    axios.all([axios.patch(url, {newSport: addThisSport} ), axios.patch(url2, {newUserInSport: addUserToSport})])
      .then(axios.spread((...responses) => {
